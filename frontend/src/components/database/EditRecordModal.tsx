...
import { X, Save, Key, Link, AlertCircle } from 'lucide-react'
...
export function EditRecordModal({ schema, record, onClose, onSave }: EditRecordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const pkValue = record[primaryKey.name]
      const response = await fetch(`/api/database/${schema.name}/${pkValue}`, {
        method: 'PUT',
        headers: {
      ...
      setErrors({
        _form: error instanceof Error ? error.message : 'Failed to update record'
      })
    } finally {
      setLoading(false)
    }
  ...
  const renderInput = (column: TableSchema['columns'][0]) => {
    ...
    const isReadonly = column.isPrimaryKey && column.type === 'serial'
  ...
  const primaryKey = schema.columns.find(col => col.isPrimaryKey)
  const recordId = primaryKey ? record[primaryKey.name] : 'Unknown'
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              Edit record #{recordId} in {schema.name}
            </h2>
          </div>
          <button
            onClick={onClose}
